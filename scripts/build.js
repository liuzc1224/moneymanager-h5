...
measureFileSizesBeforeBuild(paths.appSiteBuild).then(previousFileSizes => {
  // Remove all content but keep the directory so that
  // if you're in it, you don't end up in Trash
  fs.emptyDirSync(paths.appSiteBuild);
...
function build(previousFileSizes) {
  ...
  webpack(config).run((err, stats) => {
    ...
    if (publicUrl && publicUrl.indexOf('.github.io/') !== -1) {
      ...
      var build = path.relative(process.cwd(), paths.appSiteBuild);
      console.log('The ' + chalk.cyan(build) + ' folder is ready to be deployed.');
...
function copyPublicFolder() {
  fs.copySync(paths.appPublic, paths.appSiteBuild, {
    dereference: true,
